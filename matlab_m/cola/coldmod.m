...
%            X    - State, the first 111 states are compositions of light
%                   component A with reboiler/bottom stage as X(1) and 
%                   condenser as X(111). State X(112)is holdup in reboiler/
%                   bottom stage and X(222) is hold-up in condenser. 
%            U(1) - reflux L,
%            U(2) - boilup V,
...
% Nominal liquid holdups
    M0(1)=20;      	% Nominal reboiler holdup (kmol)
    i=2:NT-1; M0(i)=1.0*ones(1,NT-2);% Nominal stage (tray) holdups (kmol)
    M0(NT)=60;      	% Nominal condenser holdup (kmol)
% Data for linearized liquid flow dynamics (does not apply to reboiler and condenser):
    taul=0.063;     	% time constant for liquid dynamics (min) - Columnd D: reasonable value for packed column or no liquid over weir
    F0=1;	 	% Nominal feed rate (kmol/min) 
    qF0 = 1; 		% Nominal fraction of liquid in feed 
    L0=11.8615732;     	% Nominal reflux flow (from steady-state data)
    L0b=L0 + qF0*F0;	% Nominal liquid flow below feed (kmol/min)
    lambda=0;		% Effect of vapor flow on liquid flow ("K2-effect")
    V0=12.476099;V0t=V0+(1-qF0)*F0;% Nominal vapor flows - only needed if lambda is nonzero 
